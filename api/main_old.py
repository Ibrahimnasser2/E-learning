...
async def general_chat(request: GeneralChatRequest):
    ...
    try:
        ...
        # Get all files marked for general inquiry
        db = next(get_db())
        
        # Get the system general inquiry user
        general_inquiry_user = db.query(User).filter(User.username == "system_general_inquiry").first()
        if not general_inquiry_user:
            general_inquiry_user = ensure_system_users(db)
        
        general_files = db.query(UploadedFile).filter(
            UploadedFile.target_roles.contains(["general_inquiry"])
        ...
        if file_paths:
            ...
            if existing_files:
                indexing_result = rag_manager.index_available_files(existing_files, general_inquiry_user.id)
        ...
        response = rag_manager.generate_answer(
            ...
            user_id=general_inquiry_user.id,  # Using the system user's ID
            roles=["general_inquiry"],  # Only access documents marked for general inquiries
...
async def send_message(
    ...
    try:
        ...
        if current_user.role == "admin":
            allowed_files = files_query.filter(UploadedFile.filename.isnot(None)).all()
        elif current_user.role == "faculty":
            allowed_files = files_query.filter(
                or_(UploadedFile.target_roles.contains(["faculty"]),
                    and_(UploadedFile.uploader_id == current_user.id, UploadedFile.target_roles.contains(["student"])))
            ).filter(UploadedFile.filename.isnot(None)).all()
        elif current_user.role == "employee":
            allowed_files = files_query.filter(UploadedFile.target_roles.contains(["employee"])).filter(UploadedFile.filename.isnot(None)).all()
        ...
        elif current_user.role == "general_inquiry":
            allowed_files = files_query.filter(UploadedFile.target_roles.contains(["general_inquiry"])).filter(UploadedFile.filename.isnot(None)).all()
        else:
            allowed_files = []
        ...
        db_message = ChatMessage(
            ...
            response=response,
            context={"context": context_texts, "indexing_info": indexing_info}
        )
        db.add(db_message)
        db.commit()
...
async def upload_file(
    ...
    allowed_roles = ["admin", "faculty", "employee", "student", "general_inquiry"]
    ...
    if current_user.role == "admin":
        ...
        if not isinstance(target_roles, list) or target_roles != ["student"]:
            raise HTTPException(status_code=400, detail="Faculty can only upload for students")
    else:
...
async def upload_student_file(
    ...
    """
    Upload an Excel file containing student data and create user accounts for new students.
    Only admin users can access this endpoint.
    """
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Only admin users can upload student files.")
    ...
    required_columns = {"Name", "University ID", "Phone Number"}
    ...
    for _, row in df.iterrows():
        ...
        password = "kk" + phone[-5:] if len(phone) >= 5 else "kk" + phone
        ...
        db_user = User(
            ...
            password_hash=hashed_password,
            role="student"
            # TODO: Add organization when implemented
        )
        db.add(db_user)
        added += 1
    db.commit()
...
async def upload_faculty_file(
    ...
    """
    Upload an Excel file containing faculty data and create user accounts for new faculty members.
    Only admin users can access this endpoint.
    """
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Only admin users can upload faculty files.")
    ...
    required_columns = {"Name", "Account", "Phone Number"}
    if not required_columns.issubset(df.columns):
        raise HTTPException(status_code=400, detail=f"Excel file must contain columns: {', '.join(required_columns)}")

    # Remove duplicate rows based on Account and Email
    df["Account"] = df["Account"].astype(str).str.strip()
    df["Phone Number"] = df["Phone Number"].astype(str).str.strip()
    df["Email"] = df["Account"] + "@kk.edu.sa"
    df = df.drop_duplicates(subset=["Account", "Email"])
    ...
    for _, row in df.iterrows():
        ...
        password = "UU" + phone[-5:] if len(phone) >= 5 else "UU" + phone
        username = account
        # Check for existing user by username or email
        existing_user = db.query(User).filter((User.username == username) | (User.email == email)).first()
        if existing_user:
        ...
        db_user = User(
            ...
            password_hash=hashed_password,
            role="faculty"
            # TODO: Add organization_id when implemented
        )
        db.add(db_user)
        added += 1
    db.commit()
...
async def upload_staff_file(
    ...
    """
    Endpoint لتحميل ملف Excel فيه بيانات الموظفين: الاسم، الحساب، رقم الهاتف.
    يتم توليد:
        الإيميل = نفس الحساب.
        كلمة المرور = "MM" + آخر 5 أرقام من رقم الهاتف.
    يتم تجاهل أي موظف موجود مسبقًا.
    يتم حفظ فقط الجدد، وإرجاع عدد من تمت إضافتهم وعدد من تم تخطيهم.
    الوصول فقط للمسؤول (admin).
    """
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Only admin users can upload staff files.")
    ...
    # Remove duplicate rows based on Account
    df["Account"] = df["Account"].astype(str).str.strip()
    df["Phone Number"] = df["Phone Number"].astype(str).str.strip()
    ...
    for _, row in df.iterrows():
        ...
        account = str(row["Account"]).strip()
        phone = str(row["Phone Number"]).strip()
        email = account  # No domain
        password = "MM" + phone[-5:] if len(phone) >= 5 else "MM" + phone
        username = account
        # Check for existing user by username or email
        existing_user = db.query(User).filter((User.username == username) | (User.email == email)).first()
        if existing_user:
        ...
        db_user = User(
            ...
            password_hash=hashed_password,
            role="employee",
            # organization_id: assign if present on current_user
            **({"organization_id": getattr(current_user, "organization_id", None)} if hasattr(current_user, "organization_id") else {})
        )
        db.add(db_user)
        added += 1
    db.commit()
    print(f"[UPLOAD STAFF FILE] Added: {added}, Skipped: {skipped}")
    return {"added": added, "skipped": skipped}
